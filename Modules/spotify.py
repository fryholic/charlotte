...
class Downloader:
    ...
    async def _download_track(self, track, content_type, content_name):
        buffer = BytesIO()
        max_retries = 2
        retry_count = 0
        error_message = ""
        ...
        while retry_count <= max_retries:
            try:
                async with aiohttp.ClientSession() as session:
                    # API 요청
                    api_url = f"https://api.spotidownloader.com/download/{track.id}"
                    headers = {
                    ...
                    # 오디오 다운로드
                    async with session.get(data['link'], timeout=300) as audio_response:
                        if audio_response.status != 200:
                        ...
                        # 메타데이터 임베딩
                        await self._embed_metadata(buffer, track)
                        buffer.seek(0)

                        # 파일 저장
                        filepath = self._get_output_path(track, content_type, content_name)
